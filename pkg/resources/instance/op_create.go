...
func BundleSuggestFiltered(s *Store, platform *string, category *string, ipType ...*string) func(context.Context) ([]registry.Choice, error) {
	return func(ctx context.Context) ([]registry.Choice, error) {
		...
		for _, b := range bundles {
			...
			out = append(out, registry.Choice{
				Value:   b.ID,
				Display: fmt.Sprintf("%-12s  %2d vCPU  %5.1f GB RAM  %4d GB disk  %6s transfer  $%7.2f/mo", b.Name, b.VCPUs, b.RAM, b.Disk, formatTransfer(b.Transfer), b.Price),
			})
		}
		return out, nil
	}
...
func BundleCategorySuggest() func(context.Context) ([]registry.Choice, error) {
	return func(_ context.Context) ([]registry.Choice, error) {
		return []registry.Choice{
			...
			{Value: lightsail.BundleCategoryComputeOptimized, Display: "Compute-optimized     Compute-intensive workloads"},
			{Value: lightsail.BundleCategoryMemoryOptimized, Display: "Memory-optimized      Memory-intensive workloads"},
		}, nil
	}
}

func formatTransfer(gb int32) string {
	if gb >= 1024 {
		tb := float64(gb) / 1024
		if tb == float64(int(tb)) {
			return fmt.Sprintf("%d TB", int(tb))
		}
